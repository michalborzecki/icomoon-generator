...
exports.startBrowser = async function (headless) {
  return await puppeteer.launch({ args: ['--no-sandbox'], headless });
};
...
exports.closePreferences = async function (page) {
  ...
  await page.waitForSelector('.unit.btn0[type="button"]:not([disabled])');
  // wait for preferences to recaltulate (gui does not show when it is done)
...
exports.fillPreferences = async function (page, preferences) {
  ...
  await triggerPreferencesModelRecalculate(page);
};
...
async function triggerPreferencesModelRecalculate(page) {
  const inputDefs = [
    preferencesInputs.general.fontName,
    preferencesInputs.general.classPrefix,
  ];
  for (let i = 0; i < inputDefs.length; i++) {
    const input = await page.waitForXPath(`//span[normalize-space()="${inputDefs[i].label}"]/following-sibling::input`);
    await input.click();
    await input.focus();
  }
}
