...
module.exports = async function (inputPath, outputPath, config) {
  ...
  try {
    ...
    utils.log('Creating temporary download direcory...')
    downloadFolder = tmp.dirSync();
    const zipPath = path.join(downloadFolder.name, config.resultZipName);
    ...
    await new Promise((resolve, reject) => {
      ...
      const watcher = chokidar.watch(downloadFolder.name, { awaitWriteFinish: true }).on('add', function (filePath) {
        if (filePath === zipPath) {
          ...
          const extractStream = fs.createReadStream(zipPath).pipe(unzip.Extract({ path: outputPath }));
